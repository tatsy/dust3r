...
from dust3r.utils.geometry import inv, geotrf, get_med_dist_between_poses
from dust3r.cloud_opt.commons import i_j_ij, edge_str, compute_edge_scores


@torch.no_grad()
def init_from_known_poses(self, niter_PnP: int = 10, min_conf_thr: float = 3.0):
    device = self.device
...
def minimum_spanning_tree(
    ...
    score, i, j = todo.pop()
    if verbose:
        print(f' init edge ({i}*,{j}*) score={score.item():.6f}')
